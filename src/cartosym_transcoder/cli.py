...
def _create_convert_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "-o", "--output", type=Path, help="Output file (if omitted, prints to console)"
    )
    parser.add_argument(
...
def convert_command(args) -> int:
    ...
    try:
        ...
        if from_format == "cscss":
            ...
            from cartosym_transcoder.grammar.generated.CartoSymCSSGrammar import (
                CartoSymCSSGrammar,
            )
            from cartosym_transcoder.grammar.generated.CartoSymCSSLexer import (
                CartoSymCSSLexer,
            )
...
def validate_command(args) -> int:
    ...
    try:
        if ext == ".cscss":
            ...
            print(
                f"Extension non reconnue pour la validation : {input_path}",
                file=sys.stderr,
            )
            return 1
    except Exception as e:
