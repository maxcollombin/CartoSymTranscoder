...
class FontFamily(Expression):
    ...
    def get_css_font_family(self) -> str:
        ...
        return ", ".join(
            f'"{family}"' if " " in family else family for family in all_families
        )
...
class ScaleTransform(Expression):
    ...
    def to_matrix(self) -> TransformationMatrix:
        ...
        return TransformationMatrix(matrix=[sx, 0, 0, sy, cx * (1 - sx), cy * (1 - sy)])
...
class TranslationTransform(Expression):
    ...
    def to_matrix(self) -> TransformationMatrix:
        ...
        return TransformationMatrix(
            matrix=[1.0, 0.0, 0.0, 1.0, self.offset_x, self.offset_y]
        )
...
class UnitConversion:
    ...
    @classmethod
    def to_pixels(
        cls, value: float, unit: UnitType, context_size: float = 16.0
    ) -> float:
...
class UnitRange(Expression):
    ...
    def clamp(
        self, value: MeasureExpression, context_size: float = 16.0
    ) -> MeasureExpression:
...
class ResponsiveUnit(Expression):
    ...
    def calculate_responsive_value(
        self, viewport_width: float, viewport_height: float, context_size: float = 16.0
    ) -> MeasureExpression:
        ...
        # Apply constraints
        if self.min_constraint and scaled_px < self.min_constraint.to_pixels(
            context_size
        ):
            return self.min_constraint
        if self.max_constraint and scaled_px > self.max_constraint.to_pixels(
            context_size
        ):
